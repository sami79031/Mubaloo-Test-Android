package com.thracecodeinc.mubalootest.SQLite;

import android.content.ContentValues;
import android.content.Context;
import android.database.Cursor;
import android.database.sqlite.SQLiteDatabase;
import android.database.sqlite.SQLiteOpenHelper;
import android.util.Log;

import com.thracecodeinc.mubalootest.Models.Team;

import java.util.ArrayList;
import java.util.List;

public class DBHandler extends SQLiteOpenHelper {

    // Database Version
    private static final int DATABASE_VERSION = 1;

    // Database Name
    private static final String DATABASE_NAME = "teamsInfo";

    // Contacts table name
    private static final String TABLE_TEAMS = "teams";

    // Shops Table Columns names
    private static final String KEY_ID = "id";
    private static final String KEY_FIRST_NAME = "first_name";
    private static final String KEY_LAST_NAME = "last_name";
    private static final String KEY_ROLE = "role";
    private static final String KEY_PROFILE_IMG = "profileImageURL";
    private static final String KEY_TEAM_NAME = "team_name";
    private static final String KEY_TEAM_LEAD = "team_lead";

    public DBHandler(Context context) {
        super(context, DATABASE_NAME, null, DATABASE_VERSION);
    }

    @Override
    public void onCreate(SQLiteDatabase db) {
        String CREATE_CONTACTS_TABLE = "CREATE TABLE " + TABLE_TEAMS + "("
                + KEY_ID + " INTEGER PRIMARY KEY," + KEY_FIRST_NAME + " TEXT,"
                + KEY_LAST_NAME + " TEXT," + KEY_ROLE + " TEXT,"
                + KEY_PROFILE_IMG + " TEXT, " + KEY_TEAM_NAME + " TEXT,"
                + KEY_TEAM_LEAD + " INTEGER" +")";
        db.execSQL(CREATE_CONTACTS_TABLE);
    }

    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        // Drop older table if existed
        db.execSQL("DROP TABLE IF EXISTS " + TABLE_TEAMS);
        // Creating tables again
        onCreate(db);
    }

    // Adding new shop
    public void addTeam(Team team) {
        SQLiteDatabase db = this.getWritableDatabase();

        ContentValues values = new ContentValues();
        values.put(KEY_ID, team.getID());
        values.put(KEY_FIRST_NAME, team.getFirstName());
        values.put(KEY_LAST_NAME, team.getLastName());
        values.put(KEY_ROLE, team.getRole());
        values.put(KEY_PROFILE_IMG, team.getProfileImgURL());
        values.put(KEY_TEAM_NAME, team.getTeamName());
        // 1 == true ; 0 == false
        if (team.isTeamLead())
            values.put(KEY_TEAM_LEAD, 1);
        else
            values.put(KEY_TEAM_LEAD, 0);

        // Inserting Row
        db.insert(TABLE_TEAMS, null, values);
        db.close(); // Closing database connection
    }

    // Getting one member
    public Team getMember(String id) {
        SQLiteDatabase db = this.getReadableDatabase();

        Cursor cursor = db.query(TABLE_TEAMS, new String[]{KEY_ID,
                        KEY_FIRST_NAME, KEY_LAST_NAME, KEY_ROLE,
        KEY_PROFILE_IMG, KEY_TEAM_NAME, KEY_TEAM_LEAD}, KEY_ID + "=?",
                new String[]{String.valueOf(id)}, null, null, null, null);
        if (cursor != null)
            cursor.moveToFirst();

        assert cursor != null;
        String ID = cursor.getString(0);
        String fn = cursor.getString(1);
        String ln = cursor.getString(2);
        String role = cursor.getString(3);
        String img = cursor.getString(4);
        String teamName = cursor.getString(5);

        // 0 == false ; 1 == true
        boolean isTeamLead;
        if (cursor.getInt(6) == 0)
            isTeamLead = false;
        else
            isTeamLead = true;


        Team t = new Team(ID, fn, ln, role, img, isTeamLead, teamName);
        // return shop
        return t;
    }

    // Getting All Members
    public List<Team> getAllMembers() {
        List<Team> memberList = new ArrayList<Team>();
        // Select All Query
        String selectQuery = "SELECT  * FROM " + TABLE_TEAMS;

        SQLiteDatabase db = this.getWritableDatabase();
        Cursor cursor = db.rawQuery(selectQuery, null);

        // looping through all rows and adding to list
        if (cursor.moveToFirst()) {
            do {
                Team member = new Team();
                member.setID(cursor.getString(0));
                member.setFirstName(cursor.getString(1));
                member.setLastName(cursor.getString(2));
                member.setRole(cursor.getString(3));
                member.setProfileImgURL(cursor.getString(4));
                member.setTeamName(cursor.getString(5));

                // 0 == false ; 1 == true
                boolean isTeamLead;
                if (cursor.getInt(6) == 0)
                    isTeamLead = false;
                else
                    isTeamLead = true;
                member.setTeamLead(isTeamLead);


                // Adding contact to list
                memberList.add(member);
            } while (cursor.moveToNext());
        }

        // return contact list
        return memberList;
    }

    // Getting members Count
    public int getMembersCount() {
        String countQuery = "SELECT  * FROM " + TABLE_TEAMS;
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery(countQuery, null);
        int count = cursor.getCount();
        cursor.close();

        // return count
        return count;
    }

    // Updating a member
    public int updateMember(Team member) {
        SQLiteDatabase db = this.getWritableDatabase();

        ContentValues values = new ContentValues();
        values.put(KEY_FIRST_NAME, member.getFirstName());
        values.put(KEY_LAST_NAME, member.getLastName());
        values.put(KEY_ROLE, member.getRole());
        values.put(KEY_PROFILE_IMG, member.getProfileImgURL());
        values.put(KEY_TEAM_NAME, member.getTeamName());
        // 1 == true ; 0 == false
        if (member.isTeamLead())
            values.put(KEY_TEAM_LEAD, 1);
        else
            values.put(KEY_TEAM_LEAD, 0);
        // updating row
        return db.update(TABLE_TEAMS, values, KEY_ID + " = ?",
                new String[]{String.valueOf(member.getID())});
    }

    // Deleting a member
    public void deleteMember(Team member) {
        SQLiteDatabase db = this.getWritableDatabase();
        db.delete(TABLE_TEAMS, KEY_ID + " = ?",
                new String[] { String.valueOf(member.getID()) });
        db.close();
    }
}

